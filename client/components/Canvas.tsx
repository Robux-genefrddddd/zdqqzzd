...
function BlockRenderer({
  ...
  const style: React.CSSProperties = {
    ...
    border:
      block.style.borderColor && block.style.borderWidth
  ...
  const renderContent = () => {
    switch (block.type) {
      case "button":
        return (
          <button style={style} className="cursor-default">
      ...
      case "image":
        return (
          <img
            src={
              block.props?.src ||
              "https://images.unsplash.com/photo-1618005182384-a83a8e7b9b47?w=500&h=400&fit=crop"
            }
            alt="Block content"
            style={style}
              ...
              <div className="flex flex-col items-center justify-center h-96 w-96 rounded-lg border-2 border-dashed border-border/50">
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground/70 mt-1">
                    Drag blocks or use the library
                  </p>
                </div>
              </div>
            ) : (
