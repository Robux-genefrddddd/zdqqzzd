...
export function BuilderProvider({ children }: { children: React.ReactNode }) {
  ...
  const getBlock = useCallback((blockId: string): Block | null => {
  ...
  const getAllBlocks = useCallback((blocks: Block[] = canvas?.blocks || []): Block[] => {
    const result: Block[] = [];
  ...
  const updateBlockStyle = useCallback((blockId: string, style: Partial<BlockStyle>) => {
    if (!canvas) return;
  ...
  const addBlock = useCallback((block: Block, parentId?: string) => {
    if (!canvas) return;
